...
- (IBAction)showSdk:(id)sender {
    ...
    UIViewController *currentView = [wrapper currentViewController];
    UINavigationController *navigationController = [[UINavigationController alloc] initWithRootViewController:currentView];
...
- (void)viewDidLoad {
    ...
    // The delayed configuration really helps here with the right button item as we do not need to configure in the AppDelegate with 12.0.0 and above
    CMDWrapper *wrapper = [[CMDWrapper alloc] init];
    UIBarButtonItem *doneButton = [[UIBarButtonItem alloc] initWithTitle:@"Done" style:UIBarButtonItemStyleDone target:self action:@selector(done)];
    [wrapper configureSDKWithButtonItems:@[doneButton]];
}
...
- (void)setToken:(NSString *)token {
    // We now require setting the patient credentals on the main thread.
    dispatch_async(dispatch_get_main_queue(), ^{
        CMDWrapper *wrapper = [[CMDWrapper alloc] init];
        
        [wrapper startPatientSessionWithToken:token andSecret:self.secret withCompletionHandler:^(BOOL success) {
            if (success) {
